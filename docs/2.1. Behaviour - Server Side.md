...
Once `PUT /media-profiles` is requested, the Node MUST validate the request before responding. The Sender MUST change the Flow and/or the Source to meet at least one profile as soon as possible. This constraint MUST be adhered to when an external change (i.e. not from the `/media-profiles` endpoint) modifies any aspect of the stream. If the change does not satisfy the Media Profiles, the Sender MUST NOT transmit the signal over the network. The Sender is REQUIRED to take the following actions:
...
When a Sender is in this state, i.e. responding with a `205 Reset Content`, this indicates a Sender is unbound by Media Profiles and is similar to the initial state.
