...
`urn:x-nmos:cap:meta:preference` property in Constraint Sets of Receiver Capabilities indicates Constraint Set preference between Constraints Sets of the Receiver. NMOS Controller MUST remove values of this property before `PUT /constraints/active` and MAY use these values while making a decision about the intersection.

`urn:x-nmos:cap:meta:preference` property in Constraint Sets of Active Constraints indicates Constraint Set preference between Constraints Sets of the Active Constraints of the Sender. NMOS Controller MAY fill in this property in Constraint Sets basing on any additional information about Receivers. For example, if the user in any way informs NMOS Controller that Receiver D is more important than Receivers A, B, C and `urn:x-nmos:cap:meta:preference` of Receiver D states that Constraint Set 6 has higher preference than Constraints Sets 2, 3, 4, 5, then the consensus would be Constraint Sets 2, 3, 4, 5, 6 where Constraint Set 6 has the highest preference.
