...
- `Active Constraints Violation` when this Sender, the Flow or the `/transportfile` (if used) associated with this Sender or the Source associated with this Flow does not satisfy the Active Constraints. When the Sender is in this state, it MUST NOT allow [IS-05][IS-05] activations.
...
## Preventing breaking restrictions
...
If Receiver activation is requested with a `/transportfile` breaking Receiver Capabilities, the Receiver is REQUIRED to refuse such request.
