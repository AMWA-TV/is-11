...
An Input MAY support Base EDID. If an Input does not support it, `base_edid_support` property of such Input MUST be `false`, `GET` requests to `/base/edid` resource MUST be responded with `204 No Content`, `PUT` and `DELETE` requests to `/base/edid` resource MUST be responded with `405 Method Not Allowed`.
