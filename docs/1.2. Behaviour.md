...
- A `DELETE` request MUST clear the last successfully applied Media Profiles array. It turns Media Profiles to the initial state which exclude Sink Metadata Processing out of the process. It MUST be performed by client after disconnecting all Receivers from an according Sender.
